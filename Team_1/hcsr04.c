...
static int __init hcsr04_module_init(void)
{
    ...
    alloc_chrdev_region(&hcsr04_dev, 0, 1, "hcsr04_dev_ver2d");
	...
	if(device_create(my_class, NULL, hcsr04_dev, NULL, "hcsr04_dev_ver2d") == NULL) {
    ...
    if( gpio_request( SPEAKER, "hcsr04_dev" ) )
    {
        printk( KERN_INFO "hcsr04_dev: %s unable to get GPIO_OUT\n", __func__ );
        ret = -EBUSY;
	goto Done;
...
static void __exit hcsr04_module_cleanup(void)
{
	...
	device_destroy(my_class,hcsr04_dev);
