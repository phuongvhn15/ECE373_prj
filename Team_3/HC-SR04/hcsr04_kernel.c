...
#define SPEAKER     17 	        // SPEAKER

static dev_t hcsr04_etx_device;		//defines structure to hold major and minor number of device//
...
int hcsr04_open(struct inode *inode, struct file *file)		//opens module connecton, asserts "busy"//
{
    ...
    printk( KERN_INFO "hcsr04_etx_device: %s\n", __func__ );
    if( hcsr04_lock > 0 )
    {
	ret = -EBUSY;
    }
...
ssize_t hcsr04_write(struct file *filp, const char *buffer, size_t length, loff_t * offset)
{
    ...
    if(*buffer == 1){
	    ...
	    printk("HCSR04 has been turned off by writing a 0 value");
...
//*buf = 100;
    ...
    *buf = (int)ktime_to_us( ktime_sub( falling, rising ))/58.0;
	if((int)*buf <1130)
	{
...
static int __init hcsr04_module_init(void)
{
    ...
    alloc_chrdev_region(&hcsr04_etx_device, 0, 1, "hcsr04_etx_device_ver2d");
    printk(KERN_INFO "%s\n", format_dev_t(buffer, hcsr04_etx_device));
	...
	if(device_create(my_class, NULL, hcsr04_etx_device, NULL, "hcsr04_etx_device_ver2d") == NULL) {
    ...
    cdev_add(&hcsr04_cdev, hcsr04_etx_device, 1);


    if( gpio_request( GPIO_OUT, "hcsr04_etx_device" ) )
    {
        printk( KERN_INFO "hcsr04_etx_device: %s unable to get GPIO_OUT\n", __func__ );
        ret = -EBUSY;
	goto Done;
    }

    if( gpio_request( GPIO_IN, "hcsr04_etx_device" ) )
    {
	printk( KERN_INFO "hcsr04_etx_device: %s unable to get GPIO_IN\n", __func__ );
	ret = -EBUSY;
	goto Done;
    }

    if( gpio_request( SPEAKER, "hcsr04_etx_device" ) )
    {
        printk( KERN_INFO "hcsr04_etx_device: %s unable to get GPIO_OUT\n", __func__ );
        ret = -EBUSY;
	goto Done;
    ...
    if( gpio_direction_output( SPEAKER, 0 ) < 0 )
    {
        printk( KERN_INFO "hcsr04_etx_device: %s unable to set GPIO_OUT as output\n", __func__ );
	ret = -EBUSY;
	goto Done;
    }
    ...
    {
        printk( KERN_INFO "hcsr04_etx_device: %s unable to set GPIO_IN as input\n", __func__ );
        ret = -EBUSY;
        goto Done;
    }
...
{
    printk(KERN_INFO "Cleaning-up hcsr04_etx_device.\n");
	...
	device_destroy(my_class,hcsr04_etx_device);
	class_destroy(my_class);
    unregister_chrdev_region( hcsr04_etx_device, 1 );
...
MODULE_AUTHOR("Thanh Huy - Van Y - Quoc Kien - Dai Viet");
MODULE_LICENSE("GPL");